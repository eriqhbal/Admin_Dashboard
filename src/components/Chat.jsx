    ...
    <div className="nav-item absolute right-5 md:right-52 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96">
      <div className="flex justify-between items-center">
        <div className="flex gap-3 px-2">
          ...
          <button type="button" className="text-white text-xs rounded p-1 px-2 bg-orange-400">4</button>
        </div>
        <Button
        ...
        size="2xl"
        borderRadius="50%"
        currentFunction={() => controlClick('chat')}/>
      </div>
      <div className="mt-5">
        {chatData?.map((item, index) => (
          <div key={index} className="flex items-center gap-5 border-b-1 border-color p-3 leading-8 cursor-pointer ">
            <div className="relative">
              ...
              <span style={{ background: '##5B8FB9'}} className="absolute inline-flex rounded-full h-2 w-2 right-0 -top-1"/>
            </div>
            <div>
